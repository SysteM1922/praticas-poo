...
public class P002021 {
	...
	private static void alinea2(PrintStream out) {
		out.println("\nAlínea 2) ----------------------------------\n");
		AgenciaTuristica fest = new AgenciaTuristica("Tours4Eating", "Universidade de Aveiro");

// Adicione a seguir o código necessário para a leitura do ficheiro
		try{
			Scanner input = new Scanner(new File("src/103600/uafest.txt"));
        	input.nextLine();
			while(input.hasNextLine() && !input.nextLine().equals("")){
				String [] s =input.nextLine().split(";");
				if(s[0].equals("1"))
					fest.add(new Gastronomia(Integer.parseInt(s[1]), s[2]));
				else{
					String[] se = new String[s.length-3];
					int e=0;
					for(int i=3; i<s.length; i++){
						se[e++]=s[i];
					}
					fest.add(new PasseioBicicleta(Integer.parseInt(s[1]), s[2], se));
				}
			}
		}catch(IOException e){
